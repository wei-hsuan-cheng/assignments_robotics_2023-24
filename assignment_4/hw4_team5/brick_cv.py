...
def Regress():
    camera_mat, camera_distort = \
        ReadCameraParam(f"{DIR}/camera_params.npy")
    ...
    for img_idx in range(IMG_NUM):
        ...
        for obj_idx in range(OBJ_NUM):
            ...
            obj_loc_cases.append(
                Ts_base_to_gripper[img_idx] @ obj_loc)
    ...
    print(f"obj_loc_cases[0].shape = {obj_loc_cases[0].shape}")

    obj_loc_cases = np.stack(obj_loc_cases, axis=-1) # [4, 1, N]
    assert len(obj_loc_cases.shape) == 3
    assert obj_loc_cases.shape[:2] == (4, 1)
    obj_loc_cases = obj_loc_cases.reshape((4, -1))
    # [4, N]

    obj_img_cases = np.stack(obj_img_cases, axis=-1) # [4, 1, N]
    assert len(obj_img_cases.shape) == 3
    assert obj_img_cases.shape[:2] == (4, 1)
    obj_img_cases = obj_img_cases.reshape((4, -1))
    # [4, N]

    obj_M = SolveM(obj_img_cases, obj_loc_cases)

    print(f"obj_M\n{obj_M}")

    np.save(f"{DIR}/obj_M.npy", {"obj_M": obj_M})
...
def Inference(img, cur_pose):
    ...
    obj_area_factors = np.array([2, 2, 3])

    OBJ_NUM = obj_colors.shape[0]

    obj_M = NPLoad(f"{DIR}/obj_M.npy") # [4, 4]
    obj_M = obj_M["obj_M"]
    ...
    tmp = obj_M @ obj_camera_locs # [OBJ_NUM, 4, 1]
