...
fprintf('=====HW2, Part C-2 (B), IK-case 1=====\n');

%%%%%%%%%% Target pose
q_target = [600, 100, 50, pi/4, 0, pi].';
p05 = q_target(1 : 3);
...
r = sqrt(xw^2 + yw^2);
zw_prime = zw - d1;
r_prime = sqrt(r^2 - d2^2) - a1;

%%%%% Solve for th1, th2 and th3
lambda1 = atan2(-d2, sqrt(r^2 - d2^2)); lambda1d = lambda1 * r2d;
lambda2 = atan2(zw_prime, r_prime); lambda2d = lambda2 * r2d;
D2 = (a2^2 + r_prime^2 + zw_prime^2 - a3^2) / (2 * a2 * sqrt(r_prime^2 + zw_prime^2));
D3 = (r_prime^2 + zw_prime^2 - a2^2 - a3^2) / (2 * a2 * a3);

th1 = atan2(yw, xw) - lambda1; th1d = th1 * r2d;
